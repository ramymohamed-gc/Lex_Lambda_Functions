...
def fulfill_intent(event, intent_name, slots):
    ...
    """
    # Example
    if intent_name == 'OutlookDelegateAccess':
        OutlookVersion = slots['OutlookVersion']['value']['interpretedValue']
        PermissionsLevel = slots['PermissionsLevel']['value']['interpretedValue']
        PlatformType = slots['PlatformType']['value']['interpretedValue']
        
        # Define the user's query based on collected slots
        user_query = (
            f"I need help setting up delegate access in Outlook on {PlatformType}. "
            f"I'm using {OutlookVersion}, and I want to give {PermissionsLevel} access."
        )
        ...
        if kendra_answer:
            system_message_content = (
                ...
                "Your role is to assist users in setting up delegate access, guiding them through the steps based on "
                "their platform (e.g., Windows, Mac, Web) and Outlook version. "
            ...
            system_message_content = (
                "You are a virtual assistant specializing in helping users with technical tasks related to Outlook. "
                "Your role is to assist users in setting up delegate access, guiding them through the steps based on "
                "their platform (e.g., Windows, Mac, Web) and Outlook version. Provide clear, concise, and step-by-step "
                "instructions, and offer additional help when needed."
                "Generate a response that is clear, concise, and well-organized. Provide only the essential steps, "
                "and offer to provide more details if the user needs them."
            )
    ...
    elif intent_name == 'ResetLaptopPassword':
        FirstName = slots['FirstName']['value']['interpretedValue']
        LastName = slots['LastName']['value']['interpretedValue']
        Email = slots['Email']['value']['interpretedValue']
        
         # User query formulation based on provided information
        user_query = (
            f"I need to reset my CMD (Centrally Managed Devices) Laptop password for {FirstName} {LastName} with email {Email}."
        )
        ...
        if kendra_answer:
            system_message_content = (
                "You are a virtual assistant specializing in helping users reset their Centrally Managed Devices laptop passwords. "
                "Your role is to guide the user through the password reset process. "
                "Here is some relevant information from our internal documentation to help you: "
                f"{kendra_answer}. "
                "At the end of your response, include a 'References' section that cites the URLs associated with the information."
                "Use this information as a basis for your response."
        ...
        return guide_message
    elif intent_name == 'SetupPhone':
        PhoneType = slots['PhoneType']['value']['interpretedValue']
        SetupTask = slots['SetupTask']['value']['interpretedValue']

        
        # Define the user's query based on collected slots
        user_query = (
            f"I need help with setup task in my phone for the followings: {SetupTask}."
            f"My phone type is {PhoneType}"
        )
        ...
        if kendra_answer:
            system_message_content = (
                "You are a virtual assistant specializing in helping users with technical tasks related to setting up their phones. "
                "Your role is to assist users in setting up their phone, guiding them through the steps based on "
                "their phone type (e.g., Android, iPhone, BlackBerry) and "
                "setup task (e.g., Android Enterprise with M365, BBME setup, or iOS 17 activation). "
                "Here is some relevant information from our internal documentation to help you: "
                f"{kendra_answer}. "
                "At the end of your response, include a 'References' section that cites the URLs associated with the information."
                "Use this information as a basis for your response."
